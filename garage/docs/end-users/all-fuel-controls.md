# All Fuel Controls

![All Fuel Controls screenshot](../assets/screenshots/all-fuel-controls.png)

The All Fuel Controls page lets you manage and monitor fuel consumption data across your vehicle fleet. Operations staff use this page to track fuel efficiency, set consumption benchmarks, and identify vehicles with unusual fuel usage patterns. Access this page regularly to maintain accurate fuel records and catch maintenance issues early.

**URL:** `https://creatorapp.zoho.com/m.fahmy_ugologistics/u-go-garage-maintenance#Report:All_Fuel_Controls`

## How to Use

1. Select your language using the lang-select-dropdown at the top if needed.
2. Use the search field (s2id_autogen80_search) to find a specific vehicle by name or ID.
3. Review the table to see each vehicle's consumption data: Normal Consumption Rate, Total Liters, Average Consumption, Last Odometer, and Cost Per KM.
4. Check the checkboxes next to vehicles to select multiple records for bulk actions like editing or deleting.
5. Click Bulk Edit to update consumption rates or other settings for selected vehicles at once.
6. Use Export to download fuel data for reports, or Import to add new vehicle fuel records in bulk.
7. Click Done when finished, or navigate back using Main Forms or other menu buttons.

## Page Sections

- All Fuel Controls

## Form Fields

| Field | Description | Type | Required |
|-------|-------------|------|----------|
| lang-select-dropdown |  | dropdown | No |
| Checkbox |  | checkbox | No |
| 4780709000000835067_checkbox |  | checkbox | No |
| 4780709000000832024_checkbox |  | checkbox | No |
| 4780709000000832019_checkbox |  | checkbox | No |
| 4780709000000832006_checkbox |  | checkbox | No |
| 4780709000000805053_checkbox |  | checkbox | No |
| 4780709000000805048_checkbox |  | checkbox | No |
| 4780709000000805043_checkbox |  | checkbox | No |
| 4780709000000805036_checkbox |  | checkbox | No |
| 4780709000000805022_checkbox |  | checkbox | No |
| 4780709000000805012_checkbox |  | checkbox | No |
| 4780709000000797013_checkbox |  | checkbox | No |
| Checkbox |  | checkbox | No |
| wrapColsInputEl |  | checkbox | No |
| show-hide-col-all |  | checkbox | No |
| viewdel |  | checkbox | No |
| viewdel |  | checkbox | No |
| viewdel |  | checkbox | No |
| viewdel |  | checkbox | No |
| viewdel |  | checkbox | No |
| viewdel |  | checkbox | No |
| volume | A range slider to filter or adjust fuel volume settings for selected vehicles. | range | No |
| checkbox-Vehicle-ZC_P8JL86 |  | checkbox | No |
| s2id_autogen80 |  | text | No |
| s2id_autogen80_search |  | text | No |
| zc_searchSelect |  | dropdown | No |
| checkbox-Normal_Consumption_Rate-ZC_P8JL86 |  | checkbox | No |
| s2id_autogen82 |  | text | No |
| s2id_autogen82_search |  | text | No |
| zc_searchSelect |  | dropdown | No |
| From |  | text | No |
| To |  | text | No |
| checkbox-Total_Liters-ZC_P8JL86 |  | checkbox | No |
| s2id_autogen84 |  | text | No |
| s2id_autogen84_search |  | text | No |
| zc_searchSelect |  | dropdown | No |
| From |  | text | No |
| To |  | text | No |
| checkbox-Average_Consumption-ZC_P8JL86 |  | checkbox | No |
| s2id_autogen86 |  | text | No |
| s2id_autogen86_search |  | text | No |
| zc_searchSelect |  | dropdown | No |
| From |  | text | No |
| To |  | text | No |
| checkbox-Last_Odometer-ZC_P8JL86 |  | checkbox | No |
| s2id_autogen88 |  | text | No |
| s2id_autogen88_search |  | text | No |
| zc_searchSelect |  | dropdown | No |
| From |  | text | No |
| To |  | text | No |
| checkbox-Cost_Per_KM-ZC_P8JL86 |  | checkbox | No |
| s2id_autogen90 |  | text | No |
| s2id_autogen90_search |  | text | No |
| zc_searchSelect |  | dropdown | No |
| From |  | text | No |
| To |  | text | No |
| allowlocation |  | button | No |
| useIconSwitch |  | checkbox | No |
| toggleIconSwitch |  | checkbox | No |
| saveIcons |  | button | No |
| resetIcons |  | button | No |
| Search... |  | text | No |
| I have read the above conditions. |  | checkbox | No |
| reqEmailId |  | text | No |
| s2id_autogen2 |  | text | No |
| s2id_autogen2_search |  | text | No |
| supportType |  | dropdown | No |
| Please enable edit permission to help with troubleshooting |  | checkbox | No |
| reachusChatDescription |  | textarea | No |
| reachUsStartChat |  | button | No |

## Table Columns

- Vehicle
- Normal Consumption Rate
- Total Liters
- Average Consumption
- Last Odometer
- Cost Per KM

## Actions

- **Done**
- **Main Forms**
- **Master Data**
- **Maintenance Sub Forms**
- **Spare Parts Sub Forms**
- **Support Forms**
- **Remove Changes**
- **Show as**
- **Print**
- **Import**
- **Export**
- **Bulk Edit**
- **Duplicate**
- **Delete**
- **AddAdd (Option + Shift + N)**
- **Submit Request**

## Tips

- If Average Consumption is significantly higher than Normal Consumption Rate, the vehicle may need maintenance—check the Maintenance Sub Forms to schedule a service.
- Always export your fuel data before making bulk changes; this creates a backup in case you need to undo modifications.

## Related Pages

- [Fuel Control](fuel-control.md)
- [Fuel Entries](fuel-entries.md)
- [All Fuel Entries](all-fuel-entries.md)
- [Fuel – Vehicle Performance Summary](fuel-vehicle-performance-summary.md)
- [Worst Vehicles Report](worst-vehicles-report.md)
- [Cost Per KM](cost-per-km.md)

## Common Workflows

_Add step-by-step workflows specific to your organisation here._
